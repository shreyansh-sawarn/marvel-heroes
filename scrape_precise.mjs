...
async function scrapePrecise() {
  ...
  const queries = [
    { q: 'site:wallpapercave.com "spiderman-homecoming" 4k', tag: 'wc_homecoming' },
    { q: 'site:wallpapercave.com "spider-man-far-from-home" 4k', tag: 'wc_ffh' },
    { q: 'site:wallpapercave.com "spider-man-no-way-home" 4k', tag: 'wc_nwh' },
    { q: 'Tom Holland "Spider-Man: Homecoming" official movie still 4k', tag: 'still_hc' },
    { q: 'Tom Holland "Far From Home" swing mid-air 4k', tag: 'still_ffh' }
  ];

  for (const item of queries) {
    console.log(`Searching Bing for: ${item.q}...`);
    await page.goto(`https://www.bing.com/images/search?q=${encodeURIComponent(item.q)}&qft=+filterui:imagesize-wallpaper`, { waitUntil: 'domcontentloaded' });
  ...
  await browser.close();
  console.log('Precise scrape done!');
}

scrapePrecise().catch(console.error);
