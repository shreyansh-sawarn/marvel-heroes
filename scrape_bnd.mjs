...
async function scrapeBrandNewDay() {
  ...
  const queries = [
    { q: '"Spider-Man: Brand New Day" Tom Holland 4k wallpaper', tag: 'bnd_hero' },
    { q: '"Spider-Man: No Way Home" final swing suit 4k wallpaper', tag: 'final_swing' },
    { q: 'site:hdqwalls.com "spider man no way home final suit" 4k', tag: 'hdq_final' },
    { q: 'site:wallpaperaccess.com "spider man brand new day" 4k', tag: 'wa_bnd' },
    { q: 'site:wallpapercave.com "spider man no way home final swing" 4k', tag: 'wc_final' },
    { q: 'Tom Holland Spider-Man final suit classic blue and red 4k', tag: 'classic_mcu' }
  ];
  ...
  for (const item of queries) {
    ...
    try {
      ...
      let count = 0;
      for (let i = 0; i < imgData.length && count < 8; i++) {
        const it = imgData[i];
  ...
  await browser.close();
  console.log('Brand New Day scrape complete!');
}

scrapeBrandNewDay().catch(console.error);
